...
Mozcで入力していると、上段の記号類が正しく入力出来ない場合があります。これはMozcが利用するキーボードが「既定の入力メソッド」になっている場合に起こるようです。入力メソッドの設定画面で「Mozc」をダブルクリックすると「キーボートレイアウト」が選択出来ますので、これを「英語（US）」にしてみてください。
